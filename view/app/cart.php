            ...
            <div class="col-12 col-lg-8">
                <div class="d-flex align-items-center justify-content-between mb-3" style="font-size:13px;color:var(--text-secondary)">
                    <label class="d-flex align-items-center gap-2" style="cursor:pointer">
                        <input type="checkbox" id="checkAll" onchange="cartToggleAll(this)" style="width:16px;height:16px;accent-color:var(--btn-primary)">
                        <span>Chọn tất cả (<span id="tongSoItem"><?= count($cartItems) ?></span> sản phẩm)</span>
                    </label>
                    <button onclick="cartRemoveSelected()" style="background:none;border:none;color:var(--error-color);font-size:13px;cursor:pointer;padding:0;font-weight:600">
                ...
                <div id="cart-items-list">
                    ...
                    <div class="cart-item" id="item-<?= $sp['listing_id'] ?>" style="transition: all 0.3s ease;">
                        <input type="checkbox" class="item-check" value="<?= $sp['listing_id'] ?>" 
                               ...
                               onchange="cartUpdateSummary(); cartUpdateOrderBtn()">
                            ...
                            <div class="item-qty-wrapper">
                                <div class="item-quantity">
                                    <?php if(!empty($sp['offer_id'])): ?>
                                        ...
                                        <button class="qty-btn" disabled>+</button>
                                    <?php else: ?>
                                        <button class="qty-btn" onclick="cartChangeQty(this, <?= $sp['listing_id'] ?>, -1)">−</button>
                                        <input type="number" value="<?= $sp['quantity'] ?>" class="qty-input" 
                                               id="qty-<?= $sp['listing_id'] ?>" 
                                               data-dongia="<?= $sp['price_snapshot'] ?>"
                                               data-stock="<?= (int)$sp['stock_quantity'] ?>"
                                               onchange="cartManualChangeQty(this, <?= $sp['listing_id'] ?>)"
                        ...
                        <button class="btn-remove" onclick="cartRemoveItem(<?= $sp['listing_id'] ?>)"><i class="bi bi-trash fs-5"></i></button>
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
                ...
                <div class="cart-summary">
                    ...
                    <div class="mt-3 mb-3">
                        <div class="d-flex gap-2">
                            ...
                            <button onclick="cartApplyVoucher()" id="voucher-btn" style="border-radius:8px;padding:10px 14px;font-size:13px;white-space:nowrap;background-color:var(--bg-main);color:var(--text-primary);border:1px solid var(--border-color);font-weight:600;cursor:pointer">Áp dụng</button>
                        </div>
                    ...
                    <form id="checkoutForm" action="index.php?controller=checkout" method="POST">
                        ...
                        <button type="button" id="orderBtn" onclick="cartGoToCheckout()" 
