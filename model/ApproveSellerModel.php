...
class ApproveSellerModel {
    ...
    public function rejectSellerProfile($profileId, $userId, $reason) {
        try {
            ...
            // 1. Tạo thông báo lỗi giải thích chi tiết lý do từ chối gửi cho User
            ...
            // 2. Xóa bản ghi đăng ký cũ trong bảng seller_profiles để giải phóng ràng buộc UNIQUE(user_id)
            $sqlDelete = "DELETE FROM seller_profiles WHERE id = :profile_id";
            $stmtDelete = $this->conn->prepare($sqlDelete);
            $stmtDelete->execute([':profile_id' => $profileId]);
